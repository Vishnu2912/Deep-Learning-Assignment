...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_tr = x_train.reshape(num_train,-1)
  x_te = x_test.reshape(num_test,-1)
  for i in range(num_train):
    for j in range(num_test):
      dists[i,j] = torch.sqrt(torch.sum((x_tr[i]-x_te[j])**2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_te = x_test.reshape(num_test,-1)
  for i in range(num_train):
    dists[i,:] = torch.sqrt(torch.sum((x_tr[i]-x_te)**2,axis=1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_tr = x_train.reshape(num_train,-1)
  x_te = x_test.reshape(num_test,-1)
  dists = torch.sqrt((torch.sum(x_tr**2,dim = 1,keepdims = True) -2*torch.mm(x_tr,x_te.t()) + torch.sum(x_te.t()**2,dim = 0,keepdims = True)))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  values,idxs = torch.topk(dists,k,dim=0,largest=False)
  y_temp = y_train[idxs.reshape(1,-1)]
  y_temp = y_temp.reshape(k,-1)
  y_pred,_ = torch.mode(y_temp,dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
