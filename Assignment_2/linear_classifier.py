...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #print(correct_class_score.view(-1,1))
  margins = scores - correct_class_score.view(-1,1) + 1
  margins[range(num_train),y] = 0
  #print(margins)
  pos_indx = (margins > 0)
  loss += margins[pos_indx].sum()
  loss /= num_train
  loss += reg*torch.sum(W*W)
  ...
  #############################################################################
  # Replace "pass" statement with your code
  indx = torch.zeros_like(margins,dtype=torch.bool)
  binary = torch.zeros_like(margins)
  binary[pos_indx] = 1
  binary[range(num_train),y] = 0
  #print(binary)
  yi_sum = torch.sum(binary, axis=1)
  #print(yi_sum.shape)
  binary[range(num_train),y] -= yi_sum
  #print(X.t().shape)
  #print(binary.shape)
  dW = X.t().mm(binary)
  dW /= num_train
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  indx = torch.randint(num_train, (batch_size,))
  y_batch = y[indx]
  X_batch = X[indx]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-5,1e-4,1e-3,5e-2,1e-2]
  regularization_strengths = [1e-2, 1e-1, 1,2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  cls.train(data_dict['X_train'],data_dict['y_train'], lr, reg, num_iters, batch_size=200, verbose = False)
  y_train_pred = cls.predict(data_dict['X_train'])
  y_val_pred = cls.predict(data_dict['X_val'])
  train_acc = (data_dict['y_train'] == y_train_pred).double().mean().item()*100
  val_acc = (data_dict['y_val'] == y_val_pred).double().mean().item()*100
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores -= max_score.view(-1,1)  #to prevent numerical instability
  e_j = torch.exp(scores)
  sum_j = torch.sum(e_j, dim=1)
  log_sum_j = torch.log(sum_j)
  correct_class_scores = scores[range(num_train),y]
  loss = torch.sum(-correct_class_scores + log_sum_j)
  coeff = torch.zeros((num_train,num_classes), dtype=torch.float64, device='cuda')
  coeff[range(num_train),y] = -1
  #print(coeff)
  minus_X = coeff.t().mm(X)
  #print(minus_X)
  dW += minus_X.t()
  dW += ((e_j/sum_j.view(-1,1)).t().mm(X)).t()
  loss /= num_train
  loss += reg*torch.sum(W*W)
...
def softmax_get_search_params():
  ...
  regularization_strengths = np.logspace(-5,0,6)
  ...
  regularization_strengths = np.logspace(-5,1,6)
  print(learning_rates)
  print(regularization_strengths)
  ###########################################################################
  #                           END OF YOUR CODE                              #
